...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Clean and load crm_cust_info to Silver
        ...
        -- Clean and load crm_prd_info to Silver
        SET @start_time = GETDATE();
        ...
        -- Clean and load crm_sales_details to Silver
        SET @start_time = GETDATE();
        ...
        SELECT
            ...
            sls_cust_id,
            CASE
                WHEN ISNUMERIC(sls_order_dt) = 1 AND LEN(sls_order_dt) = 8 THEN CAST(CAST(sls_order_dt AS VARCHAR) AS DATE)
                ELSE NULL
            END AS sls_order_dt,
            CASE
                WHEN ISNUMERIC(sls_ship_dt) = 1 AND LEN(sls_ship_dt) = 8 THEN CAST(CAST(sls_ship_dt AS VARCHAR) AS DATE)
                ELSE NULL
            END AS sls_ship_dt,
            CASE
                WHEN ISNUMERIC(sls_due_dt) = 1 AND LEN(sls_due_dt) = 8 THEN CAST(CAST(sls_due_dt AS VARCHAR) AS DATE)
                ELSE NULL
            END AS sls_due_dt,
            CASE
            ...
            CASE
                WHEN sls_price IS NULL OR sls_price <= 0
                    THEN sls_sales / COALESCE(sls_quantity, 1)
                ELSE sls_price
            END AS sls_price
        FROM
        ...
        -- Clean and load erp_cust_az12 to Silver
        SET @start_time = GETDATE();
        ...
        -- Clean and load erp_loc_a101 to Silver
        SET @start_time = GETDATE();
        ...
        -- Clean and load erp_px_cat_g1v2 to Silver
        SET @start_time = GETDATE();
